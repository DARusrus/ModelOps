...
function config() {
  ...
  if (process.env.RUN_MODELOPS_BROWSER_E2E !== 'true' || process.env.SUPABASE_INTEGRATION_CONFIRMATION !== CONFIRMATION) {
    throw new Error(`Refusing to create browser-test data. Set RUN_MODELOPS_BROWSER_E2E=true and SUPABASE_INTEGRATION_CONFIRMATION=${CONFIRMATION}.`);
  }
...
export async function removeBrowserFixture(fixture: BrowserFixture | undefined) {
  if (!fixture) return;
  await fixture.admin.from('organizations').delete().eq('id', fixture.organizationId);
  await fixture.admin.auth.admin.deleteUser(fixture.user.id);
}
