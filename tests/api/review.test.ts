...
vi.mock('../../src/lib/idempotency', () => ({
  ...
  completeIdempotency: mocks.complete,
...
describe('POST /api/modelops/[id]/review', () => {
  beforeEach(() => {
    ...
    mocks.fingerprint.mockReturnValue('review-fingerprint');
  });

  it('normalizes the one-row composite RPC array before returning its validated public DTO', async () => {
    const attestation = {
    ...
    const client = {
      ...
      rpc: vi.fn().mockResolvedValue({ data: [attestation], error: null }),
    };
