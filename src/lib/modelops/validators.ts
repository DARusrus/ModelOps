...
export const TrainingMetadataSchema = z.object({
  epochs: z.coerce.number().min(1).max(10000).optional(),
  optimizer: z.string().max(MAX_SHORT_STRING).optional(),
  learning_rate: z.coerce.number().min(0).max(10).optional(),
  batch_size: z.coerce.number().min(1).max(100000).optional(),
});

export const HardwareMetadataSchema = z.object({
  gpu: z.string().max(MAX_SHORT_STRING).optional(),
  cpu: z.string().max(MAX_SHORT_STRING).optional(),
  ram: z.string().max(MAX_SHORT_STRING).optional(),
});

export const DeploymentMetadataSchema = z.object({
  platform: z.string().max(MAX_SHORT_STRING).optional(),
  latency: z.string().max(MAX_SHORT_STRING).optional(),
  memory: z.string().max(MAX_SHORT_STRING).optional(),
  environment: z.string().max(MAX_SHORT_STRING).optional(),
});

export const RiskAssessmentMetadataSchema = z.object({
  ethics: z.string().max(MAX_STRING_LENGTH).optional(),
  bias: z.string().max(MAX_STRING_LENGTH).optional(),
  limitations: z.string().max(MAX_STRING_LENGTH).optional(),
...
export const ExperimentMetadataSchema = z.object({
  ...
  training: TrainingMetadataSchema.optional(),
  hardware: HardwareMetadataSchema.optional(),
  deployment: DeploymentMetadataSchema.optional(),
  risk_assessment: RiskAssessmentMetadataSchema.optional(),
});
