...
export default function RunComparison({ runA, run2, sessionHistory = [], onClose }: RunComparisonProps) {
  ...
  const [requestedBaselineId, setRequestedBaselineId] = useState('');
  ...
  const selectedBaselineId = candidates.some((card) => card.record_id === requestedBaselineId) ? requestedBaselineId : '';
  const baseline = candidates.find((card) => card.record_id === selectedBaselineId) || null;
  const activeComparison = comparison?.baselineId === selectedBaselineId && comparison.candidateId === candidate?.record_id
    ? comparison.value
    : null;
  ...
  const compare = async (baselineId: string) => {
    ...
    try {
      ...
      setComparison({ baselineId, candidateId: candidate.record_id, value: body.comparison });
    } catch (cause) {
