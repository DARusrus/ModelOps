...
export default function RunComparison({ runA, run2, sessionHistory = [], onClose }: RunComparisonProps) {
  ...
  const compare = async (baselineId: string) => {
    ...
    setComparison(null);
    try {
      const response = await fetch('/api/modelops/compare', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ baseline_id: baselineId, candidate_id: candidate.record_id }) });
