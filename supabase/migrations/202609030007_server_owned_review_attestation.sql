-- Browser JWT context is intentionally not used by the privileged review RPC.
-- Only the server's secret-key role can call it; the server derives actor ID
-- from the validated user session before invoking this function.
...
begin
  ...
  select role::text into current_role from public.memberships where organization_id = card.organization_id and user_id = requesting_actor;
  if current_role is null then raise exception 'FORBIDDEN'; end if;
  if requested_action = 'submitted' then
    if current_role not in ('editor', 'reviewer', 'admin') or card.workflow_state not in ('draft', 'changes_requested') then raise exception 'FORBIDDEN'; end if;
  else
