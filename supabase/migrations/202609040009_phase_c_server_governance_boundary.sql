-- Phase C completion: server-owned mutation boundary and policy-bound attestations.
-- Apply after 202609030008_review_transition_server_authorized.sql.

alter table public.model_cards
  add column if not exists governance_policy_id text not null default 'enterprise_general';

alter table public.review_attestations
  add column if not exists policy_id text not null default 'enterprise_general';

-- Browser sessions may read their tenant's records, but all mutations now go
-- through authenticated application routes using the server-only service key.
drop policy if exists "editors create cards" on public.model_cards;
drop policy if exists "members create cards" on public.model_cards;
revoke insert, update, delete on public.model_cards, public.evidence_items, public.audit_events, public.review_attestations from authenticated;

drop function if exists public.attest_model_card_as(uuid, uuid, text, text);
...
begin
  ...
  values (card.organization_id, requesting_actor, 'model_card_attested', jsonb_build_object('action', requested_action, 'card_id', card.id, 'digest', event_digest, 'policy_id', requested_policy_id));
